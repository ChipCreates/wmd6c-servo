 ...
 *
 * Architecture:
 *   USB interrupt handler (USB_UCPD1_2_IRQHandler) processes endpoint events
 *   and fills/drains the packet memory area (PMA). Received bytes are placed
 ...
 *
 * STM32G0B1 USB IP (second generation — USB_DRD_FS):
 *   This device uses the second-generation ST USB device IP. The PMA is
 *   accessed linearly (1:1, no stride doubling). Buffer descriptor table
 *   (BDT) entries use 32-bit TXBD/RXBD fields: upper 16 bits = buffer
 *   address in PMA (byte offset), lower 16 bits = count/configuration.
 *   Endpoint registers are 32-bit CHEP0R–CHEP7R; the peripheral instance
 *   pointer is USB_DRD_FS (not the first-gen 'USB').
 *   See docs/hardware/usb-pma-stm32g0-port.md for porting details.
 *
 * USB descriptors:
 *   Device descriptor:    bcdUSB 2.0, CDC class, VID 0x0483 (ST demo), PID 0x5740
 *   Configuration:        2 interfaces (CDC control + CDC data), 3 endpoints
 *   CDC class descriptors: Header, Call Management, ACM, Union
 *
 * Packet Memory Area (PMA) layout (2KB total, 288 bytes used):
 *   0x000–0x017  BDT: 3 endpoint descriptors × 8 bytes (TXBD + RXBD)
 *   0x020–0x05F  EP0 TX buffer  (64 bytes)
 *   0x060–0x09F  EP0 RX buffer  (64 bytes)
 *   0x0A0–0x0DF  EP1 TX buffer  (64 bytes) — CDC IN (device→host, telemetry)
 *   0x0E0–0x11F  EP2 RX buffer  (64 bytes) — CDC OUT (host→device, commands)
 *
...
/* =========================================================================
 * PMA layout constants
 *
 * PMA offsets are byte offsets into the USB SRAM at USB_DRD_PMAADDR.
 * The BDT occupies the first 3×8 = 24 bytes (one TXBD+RXBD pair per EP).
 * Data buffers begin at offset 0x020 to keep natural alignment.
 * ========================================================================= */

#define EP0_TX_ADDR     0x020U   /* EP0 TX buffer — 64 bytes */
#define EP0_RX_ADDR     0x060U   /* EP0 RX buffer — 64 bytes */
#define EP1_TX_ADDR     0x0A0U   /* EP1 TX buffer — 64 bytes (CDC IN)  */
#define EP2_RX_ADDR     0x0E0U   /* EP2 RX buffer — 64 bytes (CDC OUT) */
#define PMA_EP_BUF_SIZE 64U

/* 64-byte RX buffer config: BLSIZE=1 (32-byte blocks), NUM_BLOCK=1 → (1+1)×32=64B.
 * Stored in the lower 16 bits of RXBD (COUNT_RX field). */
#define RXBD_COUNT_64   0x8400U

/* Helper macros for constructing BDT entries.
 * TXBD: [31:16]=addr, [15:0]=byte count to transmit.
 * RXBD: [31:16]=addr, [15:0]=RX buffer size config (BLSIZE|NUM_BLOCK). */
#define TXBD(addr, count)   (((uint32_t)(addr) << 16) | (uint32_t)(count))
#define RXBD_CFG(addr, cfg) (((uint32_t)(addr) << 16) | (uint32_t)(cfg))
#define RXBD_COUNT(rxbd)    ((rxbd) & 0x3FFU)   /* Received byte count, bits [9:0] */

/* =========================================================================
 * PMA access helpers (second-generation USB_DRD IP: linear 1:1 access)
 *
 * pma_write16 / pma_read16 access the PMA as 16-bit halfwords at the given
 * byte offset. No stride doubling — a halfword at PMA byte offset N is at
 * CPU address USB_DRD_PMAADDR + N.
 * ========================================================================= */

static inline void pma_write16(uint32_t pma_off, uint16_t val)
{
    *(__IO uint16_t *)(USB_DRD_PMAADDR + pma_off) = val;
}

static inline uint16_t pma_read16(uint32_t pma_off)
{
    return *(__IO uint16_t *)(USB_DRD_PMAADDR + pma_off);
}
...
static uint8_t     banner_pending    = 0;
 ...
 *
 * CHEPnR registers use toggle-bit semantics for STAT and DTOG fields:
 *   write 1 → toggles the bit; write 0 → leaves it unchanged.
 * VTTX (bit 7) and VTRX (bit 15) are RC_W0: write 0 clears, write 1 = no effect.
 *
 * The preservation mask 0x8F8FUL covers the non-toggle bits in bits[15:0]:
 *   bit 15 (VTRX), bits 11:8 (SETUP, UTYPE, KIND), bit 7 (VTTX), bits 3:0 (ADDR).
 * Upper bits [31:16] are host-mode only; writing 0 is safe in device mode.
 * ========================================================================= */

#define CHEP_PRESERVE_MASK  0x00008F8FUL

#define EP_STAT_DISABLED    0x00U
#define EP_STAT_STALL       0x01U
#define EP_STAT_NAK         0x02U
#define EP_STAT_VALID       0x03U

static void ep_set_tx_status(uint8_t ep, uint8_t status)
{
    volatile uint32_t *chepr = &USB_DRD_FS->CHEP0R + ep;
    uint32_t current = *chepr;
    uint32_t toggle  = ((current >> 4) & 0x3U) ^ (uint32_t)status;
    *chepr = (current & CHEP_PRESERVE_MASK) | (toggle << 4);
}
...
static void ep_set_rx_status(uint8_t ep, uint8_t status)
{
    volatile uint32_t *chepr = &USB_DRD_FS->CHEP0R + ep;
    uint32_t current = *chepr;
    uint32_t toggle  = ((current >> 12) & 0x3U) ^ (uint32_t)status;
    *chepr = (current & CHEP_PRESERVE_MASK) | (toggle << 12);
}
...
void usb_cdc_init(void)
{
    ...
    /* ---- GPIO: PA11 (USB D-), PA12 (USB D+) — AF10 for USB_DRD_FS ---- */
    RCC->IOPENR |= RCC_IOPENR_GPIOAEN;
    GPIOA->MODER = (GPIOA->MODER & ~((3U << (11*2)) | (3U << (12*2))))
                 | (2U << (11*2)) | (2U << (12*2));
    /* AF10 for PA11/PA12 on STM32G0B1 (USB FS) — DS13560 Table 12 */
    GPIOA->AFR[1] = (GPIOA->AFR[1] & ~((0xFU << ((11-8)*4)) | (0xFU << ((12-8)*4))))
                  | (10U << ((11-8)*4)) | (10U << ((12-8)*4));

    /* ---- USB peripheral (USB_DRD_FS) ---- */
    RCC->APBENR1 |= RCC_APBENR1_USBEN;

    /* Exit power-down, issue USB reset, then release */
    USB_DRD_FS->CNTR = USB_CNTR_USBRST;          /* Assert USB reset (PDWN=0) */
    for (volatile int i = 0; i < 1000; i++) { }   /* Tspec ≥ 1µs              */
    USB_DRD_FS->CNTR = 0;                          /* Release reset            */
    USB_DRD_FS->ISTR = 0;                          /* Clear any pending flags  */

    /* Buffer descriptor table at PMA byte offset 0 */
    USB_DRD_FS->BTABLE = 0;

    /* Enable USB reset and CTR (correct transfer) interrupts */
    USB_DRD_FS->CNTR = USB_CNTR_RESETM | USB_CNTR_CTRM;
...
/* =========================================================================
 * ep0_send() — queue a descriptor or status response on EP0 IN
 * ========================================================================= */
...
static void ep0_send(const uint8_t *data, uint16_t len, uint16_t requested_len)
{
    ...
    USB_DRD_PMA_BUFF[0].TXBD = TXBD(EP0_TX_ADDR, len);
...
/* =========================================================================
 * usb_handle_setup() — handle SETUP packets on EP0 (enumeration)
 * ========================================================================= */
...
static void usb_handle_setup(void)
{
    ...
    if ((bmRequest & 0x60) == 0x00) {
        ...
        case 0x05: /* SET_ADDRESS */
            ...
            USB_DRD_PMA_BUFF[0].TXBD = TXBD(EP0_TX_ADDR, 0);
            ep_set_tx_status(0, EP_STAT_VALID);
            break;
        ...
        case 0x09: /* SET_CONFIGURATION */
            ...
            if (usb_configured) {
                ...
                /* EP1 IN (bulk, CDC data to host — telemetry) */
                USB_DRD_FS->CHEP1R = (1U) | USB_EP_BULK;
                USB_DRD_PMA_BUFF[1].TXBD = TXBD(EP1_TX_ADDR, 0);
                ep_set_tx_status(1, EP_STAT_NAK);

                /* EP2 OUT (bulk, CDC data from host — commands) */
                USB_DRD_FS->CHEP2R = (2U) | USB_EP_BULK;
                USB_DRD_PMA_BUFF[2].RXBD = RXBD_CFG(EP2_RX_ADDR, RXBD_COUNT_64);
...
/* =========================================================================
 * USB_UCPD1_2_IRQHandler — USB interrupt service routine
 *
...
void USB_UCPD1_2_IRQHandler(void)
{
    ...
    if (istr & USB_ISTR_RESET) {
        ...
        USB_DRD_FS->CHEP0R = USB_EP_CONTROL;

        USB_DRD_PMA_BUFF[0].TXBD = TXBD(EP0_TX_ADDR, 0);
        USB_DRD_PMA_BUFF[0].RXBD = RXBD_CFG(EP0_RX_ADDR, RXBD_COUNT_64);

        ep_set_tx_status(0, EP_STAT_NAK);
        ep_set_rx_status(0, EP_STAT_VALID);

        USB_DRD_FS->DADDR = USB_DADDR_EF;  /* Enable function, address 0 */
    }

    /* ---- Correct transfer ---- */
    while (USB_DRD_FS->ISTR & USB_ISTR_CTR) {
        uint8_t ep_id  = USB_DRD_FS->ISTR & USB_ISTR_IDN;
        uint8_t is_out = (USB_DRD_FS->ISTR & USB_ISTR_DIR) ? 1 : 0;

        volatile uint32_t *chepr = &USB_DRD_FS->CHEP0R + ep_id;
        ...
        if (ep_id == 0) {
            if (is_out) {
                ...
                /* Clear VTRX, re-arm RX */
                *chepr = (*chepr & CHEP_PRESERVE_MASK) & ~USB_EP_VTRX;
                ep_set_rx_status(0, EP_STAT_VALID);
            } else {
                /* EP0 IN: STATUS stage or descriptor sent */
                *chepr = (*chepr & CHEP_PRESERVE_MASK) & ~USB_EP_VTTX;
                /* Apply deferred address after STATUS IN */
                if (usb_address) {
                    USB_DRD_FS->DADDR = USB_DADDR_EF | usb_address;
                    usb_address = 0;
                }
                ep_set_rx_status(0, EP_STAT_VALID);
            }
            ...
            *chepr = (*chepr & CHEP_PRESERVE_MASK) & ~USB_EP_VTTX;
            ...
            uint16_t cnt = (uint16_t)RXBD_COUNT(USB_DRD_PMA_BUFF[2].RXBD);
            uint8_t  tmp[64];
            if (cnt > 64) cnt = 64;
...
static void usb_cdc_tx(const uint8_t *data, uint16_t len)
{
    ...
    USB_DRD_PMA_BUFF[1].TXBD = TXBD(EP1_TX_ADDR, len);
    tx_busy = 1;
 ...
 *   FG:25612 Hz:2499 Tgt:25600 Err:+12 Int:-847 DAC:2034 Kp:9830(0.150) Ki:524(0.008)
...
static uint8_t *fmt_q16(uint8_t *p, int32_t q16)
{
    ...
    *p++ = '(';
    uint32_t whole = (uint32_t)q16 >> 16;
    uint32_t frac  = ((uint32_t)q16 & 0xFFFF) * 1000 / 65536;
    p = fmt_u32(p, whole);
    *p++ = '.';
    if (frac < 100) *p++ = '0';
    if (frac < 10)  *p++ = '0';
    ...
    if (cmd_prefix) {
        ...
        if (prefix == 'p') {
            ...
            if (c == '+') {
                ...
                if (t < ADJUSTED_TARGET_MAX) servo_set_target_period(t + 1);
                APPEND("f+\r\n");
            } else if (c == '-') {
                uint32_t t = g_target_period;
                if (t > ADJUSTED_TARGET_MIN) servo_set_target_period(t - 1);
                APPEND("f-\r\n");
            }
        }
...
/* =========================================================================
 * send_banner() — sent once on first host connection
 * ========================================================================= */
...
int usb_cdc_idle(void)
{
    ...
    if (banner_pending && !tx_busy)         return 0;
    if (telem_continuous && !tx_busy)       return 0;
    return 1;
}
