...
highly accurate resonator that vibrates at a known frequency. The DSR-1 Servo Control Board uses
...
## 2. The STM32G0C1's Crystal-Less USB Architecture

### 2.1 The HSI48 Internal Oscillator

The selected STM32G0C1KCU6 contains an internal 48 MHz RC oscillator called HSI48 (High
...
into the STM32G0C1. It works by continuously comparing the HSI48 frequency against
...
In practice, the STM32G0C1 crystal-less USB implementation is expected to enumerate reliably with
...
**System clock (64 MHz)**: Drives the Cortex-M0+ core, TIM2 input capture, PWM timer,
...
These two clocks are completely independent in the STM32G0C1. The USB peripheral
...
For the WM-D6C application specifically, eliminating the crystal improves Servo Control Board
...
**DFU firmware update**: When the Servo Control Board is in DFU mode (BOOT0 held, USB-C
connected), the DFU bootloader receives SOF packets and the HSI48 is trimmed.
Firmware updates are reliable.

**No USB connection**: When the Servo Control Board is running the servo loop without a USB
